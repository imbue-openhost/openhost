...
| `shm_mb` | integer | no | `0` | `/dev/shm` size in MiB (0 = podman default, 64 MiB). Apps doing heavy browser work may need ~2048. |
| `network_host` | bool | no | `false` | Share the host's network namespace instead of the isolated default. Needed by apps that forward routed packets (VPN servers). **Disables all network isolation** — the container can reach any host port. Cannot be combined with `egress`. |
| `egress` | string | no | `""` | Route **all** of the app's outbound traffic through a named egress profile (a WireGuard tunnel to an operator-configured exit — the owner's home IP, a WG VPN provider, etc.). The profile is registered on the host by the instance operator; the app repo never carries tunnel keys. Enforced fail-closed: if the tunnel drops, the app has no egress at all (no leak to the datacenter IP). See [Per-App Egress](./egress.md). Cannot be combined with `network_host`. |
