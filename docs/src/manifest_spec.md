...
The storage guard requires a minimum amount of free disk space: when free space drops below its threshold, it stops running apps until space is freed. It ships enabled by default (1500 MB) and is configured at runtime from the System page (enable/disable and the MB threshold), taking effect without a restart. A `storage_min_free_mb` value in the OpenHost config is treated only as a boot-time seed that can raise the threshold; it cannot disable the guard.
