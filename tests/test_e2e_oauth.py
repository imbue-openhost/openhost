...
Deploys the oauth-provider app, oauth-demo app, and mock-oauth-provider onto a
...
OAUTH_APP_PATH = "/home/host/openhost/apps/oauth_provider"
...
def _remove_app(session, router_url, app_name):
    app_id = app_id_for(session, router_url, app_name)
    if app_id is None:
        return
    session.post(f"{router_url}/remove_app/{app_id}", timeout=30)


@pytest.fixture(scope="module")
def oauth_app(session, router_url):
    _deploy_app(session, router_url, "oauth-provider", OAUTH_APP_PATH)
    yield
    _remove_app(session, router_url, "oauth-provider")
...
def oauth_demo_app(session, router_url):
    ...
    yield
    _remove_app(session, router_url, "oauth-demo")
...
def mock_provider_app(session, router_url):
    ...
    yield
    _remove_app(session, router_url, "mock-oauth-provider")


@pytest.fixture(scope="module")
def configured_mock_provider(session, router_url, domain, oauth_app, oauth_demo_app, mock_provider_app):
    """Configure the oauth-provider app and oauth-demo to use the deployed mock provider."""
    ...
    # Point the oauth-provider's "mock" provider at the deployed mock-oauth-provider
    r = session.post(
        f"{router_url}/oauth-provider/test/set-mock-provider-url",
        json={
            "provider": "mock",
            ...
            "redirect_uri": f"https://{domain}/api/services/v2/oauth_callback",
        },
        timeout=10,
    )
