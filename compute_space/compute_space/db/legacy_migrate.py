...
def migrate(db: sqlite3.Connection) -> None:
    ...
    # Historically, schema.sql ran after migrate() at every startup and
    # lazily created tables not otherwise touched by migrate() above.
    # Now that schema.sql is no longer executed at startup, migrate() is
    # solely responsible for bringing a legacy DB to the 0001 baseline.
    # Add IF-NOT-EXISTS statements so legacy DBs land in the same state
    # that yoyo will then record as 0001.
    db.executescript(
        """
