"""v6: introduce opaque ``app_id`` as the cross-table identity for apps.
...
class Migration0006AppIds(Migration):
    version = 6

    def up(self, db: sqlite3.Connection) -> None:  # pragma: no cover - apply() is overridden
        raise NotImplementedError("Migration0006AppIds drives execution through apply()")
    ...
    def _recreate_if_old_shape(
        ...
        """Run ``recreate_fn`` only if ``table`` still has the pre-v6 column.
    ...
    def _add_app_id_to_apps(self, db: sqlite3.Connection) -> None:
        ...
        if "app_id" in cols:
            return
        # Full table recreate so the post-v6 shape matches schema.sql exactly
