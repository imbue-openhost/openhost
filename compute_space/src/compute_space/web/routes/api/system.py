...
async def api_tokens_create(
    request: Request[Any, Any, Any], db: sqlite3.Connection
) -> Response[CreatedToken] | Response[dict[str, str]]:
    form = await request.form()
    name = (form.get("name") or "").strip() or "Untitled"
    expiry_hours_raw = (form.get("expiry_hours") or "").strip()
...
@delete("/api/tokens/{token_id:int}", status_code=200, guards=[require_owner_auth])
async def api_tokens_delete(token_id: int, db: sqlite3.Connection) -> OkResponse:
...
@post("/api/storage-guard", status_code=200, guards=[require_owner_auth])
async def toggle_storage_guard(request: Request[Any, Any, Any]) -> StorageGuardResponse:
...
def drop_docker_cache() -> Response[DropCacheOk] | Response[DropCacheError]:
    """Drop the container build cache to free disk space."""
    try:
        output = drop_docker_build_cache()
    except RuntimeError as e:
        return Response(content=DropCacheError(ok=False, error=str(e)), status_code=500)
