...
async def check_port(db: sqlite3.Connection, port: str = "") -> Response[CheckPortResponse] | Response[ErrorResponse]:
    """Check if a host port is available. Returns {port, available, used_by}."""
    try:
        port_int = int(port)
    except (ValueError, TypeError):
        return Response(content=ErrorResponse(error="port must be an integer"), status_code=400)
    ...
    return Response(
        content=CheckPortResponse(port=port_int, available=available, used_by=used_by),
        status_code=200,
        media_type=MediaType.JSON,
    )
    ...
    for key in form:
        if key.startswith("port_override."):
            ...
            try:
                ...
                return Response(
                    content=ErrorResponse(error=f"Invalid port override value for '{label}': {form[key]}"),
                    status_code=400,
                )
...
@route("/reload_app/{app_id:str}", http_method=[HttpMethod.GET, HttpMethod.POST], guards=[require_owner_auth])
async def reload_app(
    app_id: str, request: Request[Any, Any, Any], db: sqlite3.Connection, config: Config
